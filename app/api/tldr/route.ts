...
export async function POST(req: Request) {
    ...
    .map((r) => {
      const trend =
        r.sparkline.length >= 2
          ...
          : ''
      return `- ${r.title} — status: ${r.status}, cadence: ${r.cadence}. ${r.description}${trend}`
    })
    .join('\n')
  ...
  try {
    const { text } = await generateText({
      ...
      prompt: `Region: ${regionName}.\n\nReport catalog:\n${context}\n\nReturn exactly 3 to 4 key takeaways summarizing the current state of Managed Shipping analytics for this region — highlight notable trends in the numeric series, what is fresh vs. stale vs. planned, and where attention is needed. Output ONLY the takeaways, one per line, no numbering, no preamble.`,
    })
